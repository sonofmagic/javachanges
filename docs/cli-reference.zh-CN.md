...
对于 Gradle 项目，会检查 `gradle.properties`、实际 snapshot 发布版本、可选目标模块、干净 Git 工作区、settings/build 文件、检测到的模块、Gradle 命令、`maven-publish` 或 `publishing` 配置、签名配置或 Gradle 签名环境变量、仓库 URL 和凭据，以及下一步 `gradle-publish` 命令。
