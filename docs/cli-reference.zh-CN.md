...
对于 Maven 项目，会检查构建模型、当前版本、实际 snapshot 发布版本或 release tag、可选目标模块、干净 Git 工作区、Maven 命令、必需 POM 元数据、Central 发布 profiles、source/javadoc/signing 插件及其发布 goal 绑定、Central publishing plugin 配置、仓库 URL 和凭据，以及 GPG 签名输入。
